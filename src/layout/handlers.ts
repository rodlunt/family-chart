...
export function sortChildrenWithSpouses(children: Datum[], datum: Datum, data: Data) {
  ...
  return children.sort((a, b) => {
    ...
    // Order children by spouse order (ascending), independent of gender, so
    // each couple's children sit next to the matching spouse in the centered
    // spouse layout above (setupSpouses in calculate-tree.ts).
    return a_i - b_i
  })
}
